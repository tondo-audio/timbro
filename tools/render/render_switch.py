...
TILT_DEG = 28.0          # how far the lever leans in each state
# Sink the toggle's pivot below the panel so the lever's visual centre
# (mid-stem) aligns with the image's vertical centre — i.e. with the LED row.
TOGGLE_Z_OFFSET = -0.58
...
# ---------------------------------------------------------------------------
# Toggle switch — chrome lever + cap directly emerging from the panel.
# No visible base ring; pivot rotates ±TILT_DEG on X for UP / DOWN states.
# ---------------------------------------------------------------------------
bpy.ops.object.empty_add(type="PLAIN_AXES", location=(TOGGLE_X, 0, TOGGLE_Z_OFFSET))
pivot = bpy.context.active_object
pivot.name = "TogglePivot"

# Lever stem — thicker so it reads at preview scale
bpy.ops.mesh.primitive_cylinder_add(vertices=32, radius=0.045, depth=0.20,
                                     location=(TOGGLE_X, 0, 0.14))
